...
const environmentsKey = 'environments.yaml';
...
class SideCars extends Client {
    ...
    async get({ name }) {
        ...
        const environments = this._parse(configMapData, environmentsKey);

        return { name, container, volumes, volumeMounts, environments };
    }
