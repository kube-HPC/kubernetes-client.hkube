...
class Client {
     ...
     *   The timeout timer is always cleared once the attempt settles, so it never fires late.
    ...
    async withResilience(fn, label) {
        ...
        const attemptFn = async (attempt) => {
            // Handle for the timeout timer created below. Declared per-attempt (not
            // shared across retries) and always cleared in `finally`, so that:
            //  - a settled attempt (success OR failure) never leaves a pending timer
            //    keeping the event loop alive, and
            //  - the timer never fires *after* the attempt resolved, which would
            //    otherwise reject an already-unused promise (a harmless but noisy
            //    unhandled late rejection).
            let timer;
            try {
                // No timeout configured: run the call directly, nothing to clean up.
                if (!timeoutMs) {
                    return await fn();
                }
                // Race the real call against a timeout. Whichever settles first wins;
                // the timer is stored in `timer` so `finally` can cancel the loser.
                ...
                // Cancel the timeout timer for this attempt. Safe even when no timer
                // was set (timeoutMs falsy) or it already fired: clearTimeout(undefined)
                // is a no-op, and clearing an already-fired timer does nothing.
                clearTimeout(timer);
            }
            if (onError) {
