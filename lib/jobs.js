...
class Jobs extends Client {
    ...
    async update({ jobName, spec }) {
        return this._client.apis.apps.v1.namespaces(this._namespace).jobs(jobName).patch({ body: spec });
    }

    async delete({ jobName }) {
        return this._client.apis.batch.v1.namespaces(this._namespace).jobs(jobName).delete();
    }
}
