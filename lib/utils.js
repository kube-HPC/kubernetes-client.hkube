...
function _setKeyValueAnnotation(annotations, key, value) {
...
function _setKeyValueEnv(env, key, value) {
...
const applyAnnotation = (inputSpec, inputAnnotations) => {
    ...
    const { annotations } = spec.metadata;
    Object.entries(inputAnnotations).forEach(([key, value]) => {
        _setKeyValueAnnotation(annotations, key, value);
    });
    return spec;
};
